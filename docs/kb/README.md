...
| [internal/agent](packages/agent.md) | Backend interface + DefaultBackend (execution seam); PrepareExecution + ExecutionPrep (policy resolution); WriteActiveTask, GetSkillForTaskType, RunAgent, ParseSessionResult, ArchiveActiveTask; full agent lifecycle for one iteration |
