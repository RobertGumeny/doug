...
| `policy.phases.<phase>` missing the expected `interaction_mode` | Affected phase does not carry the explicit Pi-era default generated by `doug init` |

Required phase defaults: `planning` uses `interaction_mode: interactive`; `runtime`, `scaffold`, `research`, and `post_epic_kb` use `interaction_mode: rpc`.
...
Review the report output and confirm that detected drift items are expected. A freshly initialized workspace should show no drift. A pre-Pi workspace should surface retired artifacts (`.claude/`, `.codex/`, `.gemini/`), config gaps (`policy.phases` missing or incomplete), stale policy naming such as `execution_mode`, and missing `.pi/` managed surfaces.
...
- All five phases in `policy.phases` carry the explicit init defaults (`planning: interactive`; runtime/scaffold/research/post-epic KB: `rpc`)
