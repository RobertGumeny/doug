...
Research prompt text comes from Doug's built-in research prompt via `config.BuildInitialPrompt(...)`. Backend routing comes from the resolved `policy` in `.doug/doug.yaml`; after `doug init`, that policy defaults to `interaction_mode: rpc` for research runs, so Doug sends the prompt and restrictions to Pi and Pi chooses the downstream provider/model configuration.
