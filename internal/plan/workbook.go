...
func InitialPlanDocument(ctx WorkbookContext) string {
	...
	return RefreshPlanDocument(""+
		...
		"When you are ready to finalize after that confirmation, consult the reusable handoff template colocated with the `plan` skill at `.agents/skills/doug-plan/references/handoff-template.yaml`, then replace this stub with the completed schema.\n\n"+
		"```yaml\n"+
